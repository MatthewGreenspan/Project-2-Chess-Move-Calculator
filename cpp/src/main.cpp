...
int main(int argc, char* argv[]) {
  ...
  // loads opening data before the ui comes up
  loadOpeningDatabase();

  // starts sdl and text stuff
  ...
  // syncs the draw array with the board state
  boardToArray(app.board, app.pieces);

  // makes the main fixed size window
  ...
  // setup for highlights and stable ui scaling
  SDL_SetRenderDrawBlendMode(app.renderer, SDL_BLENDMODE_BLEND);
  sdlSetupRendererLogical(app.renderer, WINDOW_W, WINDOW_H);

  // loads images and fonts the gui needs
  if (!loadAssets(app)) {
    ...
    while (SDL_PollEvent(&e)) {
      if (e.type == SDL_QUIT) app.quit = true;
        ...
        if (sideBtn == 1) {
          ...
          // runs best move lookup for the current board
          ...
          if (e.button.y >= btnY && e.button.y < btnY + BUTTON_HEIGHT && e.button.x >= PADDING && e.button.x < PADDING + 120) {
            ...
            // starts dragging a spare palette piece
            app.dragFromPalette = pal;
            app.dragFrom = -1;
          } else if (sq >= 0) {
            if (app.pieces[sq] != Piece::NONE) {
              // selects a board piece and shows legal targets
              app.dragFrom = sq;
              app.dragFromPalette = -1;
              app.selectedSquare = sq;
              updateLegalMoves(app);
            } else if (app.selectedSquare >= 0) {
              // if no legal move works just clear selection
